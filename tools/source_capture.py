...
def canonical_source_url(source_id: str, requested_url: str) -> str:
    """Return a versioned configured canonical URL, or the requested URL."""
    matches = [
        item
...
def write_immutable(path: Path, data: bytes) -> bool:
    """Write one content-addressed file; reject path/content collisions."""
    if path.exists():
...
def capture(stage_id: str) -> int:
    ...
    try:
        for source in stage["sources"]:
            ...
            image_status, missing_images = _capture_tracked_images(
