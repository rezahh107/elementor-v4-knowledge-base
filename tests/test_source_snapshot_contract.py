...
def test_source_record_v3_requires_recoverable_snapshot_fields() -> None:
    ...
    errors = validate_instance(base, "source-record.schema.json", "fixture")
    assert any("response_snapshot_path" in error for error in errors)

    base.update(
        {
            "response_snapshot_path": (
                "evidence/snapshots/SRC-KB-007-01/response-" + "a" * 64 + ".html"
            ),
            "normalized_snapshot_path": (
                "evidence/snapshots/SRC-KB-007-01/normalized-" + "b" * 64 + ".txt"
            ),
    ...
    assert validate_instance(base, "source-record.schema.json", "fixture") == []
...
def test_existing_record_reuse_prevents_unchanged_content_churn(
    tmp_path, monkeypatch
) -> None:
    monkeypatch.setattr(source_capture, "ROOT", tmp_path)
    raw = b"raw"
    ...
    response_path, normalized_path = source_capture.content_addressed_paths(
        "SRC-KB-007-01", raw_hash, normalized_hash, "text/html"
    )
    source_capture.write_immutable(response_path, raw)
    source_capture.write_immutable(normalized_path, normalized)
    record = {
        "schema_version": 3,
        ...
        "image_capture_status": "not_applicable",
        "missing_image_urls": [],
        "response_snapshot_path": response_path.relative_to(tmp_path).as_posix(),
        "normalized_snapshot_path": normalized_path.relative_to(tmp_path).as_posix(),
        "response_bytes_sha256": raw_hash,
    }
    assert source_capture._valid_existing_record(
        record,
        normalized_hash,
        "https://elementor.com/help/building-query-loop-grid/",
        "not_applicable",
