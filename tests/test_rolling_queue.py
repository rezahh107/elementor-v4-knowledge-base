...
from tools.queue_common import eligible_tasks, load_queue, sha256_prefixed, transition_task, validate_queue
...
def test_unknown_dependency_is_not_eligible() -> None:
    ...
    assert eligible_tasks({"tasks": [task], "controller_policy": queue["controller_policy"]}) == []


def test_malformed_external_snapshot_is_fail_closed() -> None:
    diagnostics = reconcile(load_queue(), {"pull_requests": "bad", "workflow_runs": None})
    assert isinstance(diagnostics, list)


def test_finalize_workflow_is_scoped_to_migration_branches() -> None:
    workflow = (ROOT / ".github" / "workflows" / "finalize-stage.yml").read_text(encoding="utf-8")
    assert "migration/" in workflow
    assert "migration-" in workflow
    assert "pull_request.head.repo.full_name" in workflow
