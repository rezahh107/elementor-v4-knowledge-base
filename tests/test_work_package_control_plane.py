...
import tools.github_state_snapshot as snapshot
from tools.github_state_snapshot import (
    build_request,
...
def state(prs=None):
    ...
    return {
        ...
        "pull_requests": prs,
        "workflow_runs": [item for pull_request in prs for item in pull_request["workflow_runs"]],
    }
...
def test_ready_package_selected_and_main_sha_observed():
    result = plan(state())
    assert (result["action"], result["work_package"], result["observed_main_sha"]) == (
...
def test_exact_head_success_and_review_gate():
    ...
    assert result["gates"] == {
        "ci_state": "success",
        "review_state": "clear",
        "eligible_for_external_merge_reconciliation": True,
    }
    review = {
        "user": "reviewer",
        "state": "CHANGES_REQUESTED",
        "submitted_at": "2026-07-07T10:02:00Z",
    }
...
def test_multiple_mutation_prs_block():
    result = plan(
        state(
            [
                pr(1, labels=["work-package"]),
                pr(2, ref="migration/KB-002"),
            ]
        )
    )
    assert result["action"] == "blocked"
    assert result["reason"] == "multiple_open_mutation_prs"


def test_invalid_state_fails_closed():
    value = state()
    value["pull_requests"] = None
...
def test_pull_normalization_handles_null_head_and_rejects_bad_root():
    item = normalize_pull_requests(
        [{"number": 1, "state": "open", "draft": False, "head": None}]
    )[0]
    assert item["head_sha"] is None
    assert item["head_ref"] is None
    with pytest.raises(ValueError):
        normalize_pull_requests({"message": "bad"})


def test_runs_are_exact_head_only():
    result = normalize_workflow_runs(
        {"workflow_runs": [run(ident=1), run(head="3" * 40, ident=2)]},
        HEAD,
    )
    ...
    result = normalize_reviews(
        [
            ...
            },
            {
                "user": {"login": "r"},
                "state": "APPROVED",
                "submitted_at": "2026-07-07T10:01:00Z",
            },
        ]
    )
...
def test_list_pagination_collects_all_pages(monkeypatch):
    pages = {1: list(range(100)), 2: [100]}

    def fake_api_get(_repository, endpoint, _token):
        page = int(parse_qs(urlsplit(endpoint).query)["page"][0])
        return pages[page]

    monkeypatch.setattr(snapshot, "api_get", fake_api_get)
    assert snapshot.api_get_all_list("owner/repo", "pulls?state=open", "token") == list(
        range(101)
    )


def test_workflow_run_pagination_collects_all_pages(monkeypatch):
    pages = {
        1: {"workflow_runs": [{"id": item} for item in range(100)]},
        2: {"workflow_runs": [{"id": 100}]},
    }
    ...
    monkeypatch.setattr(snapshot, "api_get", fake_api_get)
    payload = snapshot.api_get_all_workflow_runs(
        "owner/repo",
        f"actions/runs?head_sha={HEAD}",
        "token",
    )
    assert [item["id"] for item in payload["workflow_runs"]] == list(range(101))


def test_documented_retained_artifact_is_enforced_by_workflow():
    workflow = (ROOT / ".github/workflows/work-package-control-plane.yml").read_text(
        encoding="utf-8"
    )
    documentation = (
        ROOT / "docs/operations/work-package-control-plane.md"
    ).read_text(encoding="utf-8")
    assert "retained workflow artifact" in documentation
    assert "actions/upload-artifact@" in workflow
    assert "${{ runner.temp }}/repo-state.json" in workflow
    assert "${{ runner.temp }}/work-package-plan.json" in workflow
